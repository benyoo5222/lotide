...
const assertObjectsEqual = (actual, expected) => {
  ...
  const successMessage = `✅✅✅ Objects are equal:`;
  const errorMessage = `🛑🛑🛑 Objects are not equal: `;

  eqObjects(actual, expected)
    ? console.log(
        `${successMessage} ${inspect(actual)} === ${inspect(expected)}`
      )
    : console.log(
        `${errorMessage} ${inspect(actual)} !== ${inspect(expected)}`
      );
};
